...
class BusquedaTabu {
    ...
    refinar(inicial) {
        let actual = { genes: inicial.genes.map((g) => ({ ...g })), fitness: inicial.fitness };
        let mejor = actual;
        for (let iter = 0; iter < this.config.iteracionesTabu; iter++) {
            ...
            if (this.listaTabu.length > this.config.tamListaTabu)
                this.listaTabu.shift();
    ...
    generarVecinos(c) {
        ...
        for (let i = 0; i < muestra; i++) {
            ...
            const tipos = ['docente', 'aula', 'franja'];
            for (const tipo of tipos) {
                ...
                vecinos.push({
                    crom: { genes, fitness: (0, fitness_1.calcularFitness)(genes, this.entrada) },
...
exports.BusquedaTabu = BusquedaTabu;
//# sourceMappingURL=busqueda-tabu.js.map