...
exports.calcularFitness = calcularFitness;
function calcularFitness(genes, entrada) {
    let penalizacion = 0;
    ...
    for (let i = 0; i < genes.length; i++) {
        ...
        for (let j = i + 1; j < genes.length; j++) {
            ...
            if (gen.franjaId === otro.franjaId) {
                if (gen.docenteId === otro.docenteId)
                    penalizacion += 100;
                if (gen.aulaId === otro.aulaId)
                    penalizacion += 100;
            }
        }
    }
    const docenteHoras = new Map();
